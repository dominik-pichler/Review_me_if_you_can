...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{    // Match cleaning personnel and their associated reviews and emotions}
\NormalTok{    MATCH (r:Reinigungsmitarbeiter)\textless{}{-}[:CLEANED\_BY]{-}(b:Booking){-}[:HAS\_REVIEW]{-}\textgreater{}(rev:Review){-}[:HAS\_EMOTION]{-}\textgreater{}(em:Emotion)}
\NormalTok{    WHERE em.text IN [\textquotesingle{}joy\textquotesingle{}, \textquotesingle{}disgust\textquotesingle{}] // Filter for relevant emotions}

\NormalTok{    // Aggregate emotion counts and total cleanings}
\NormalTok{    WITH r, em.text AS emotion, count(em) AS emotionCount, count(DISTINCT b) AS totalCleanings}
\NormalTok{   ORDER BY r.name}

\NormalTok{    // Calculate joy{-}to{-}disgust ratio}
\NormalTok{    WITH r, }
\NormalTok{         sum(CASE WHEN emotion = \textquotesingle{}joy\textquotesingle{} THEN emotionCount ELSE 0 END) AS joyCount,}
\NormalTok{         sum(CASE WHEN emotion = \textquotesingle{}disgust\textquotesingle{} THEN emotionCount ELSE 0 END) AS disgustCount,}
\NormalTok{         totalCleanings}

\NormalTok{    WITH r, }
\NormalTok{         joyCount, }
\NormalTok{         disgustCount, }
\NormalTok{         totalCleanings,}
\NormalTok{         CASE WHEN disgustCount = 0 THEN joyCount ELSE joyCount * 1.0 / disgustCount END AS joyDisgustRatio}

\NormalTok{    // Order by joy{-}to{-}disgust ratio to rank performers}
\NormalTok{    ORDER BY joyDisgustRatio DESC}

\NormalTok{    // Return ranked list of performers with total cleanings}
\NormalTok{     RETURN r.name AS cleaner, }
\NormalTok{            joyDisgustRatio AS ratio, }
\NormalTok{            totalCleanings}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{    // Match apartments and their associated reviews and emotions}
\NormalTok{    MATCH (a:Appartment)\textless{}{-}[:HAS\_BOOKED\_APPARTEMENT]{-}(b:Booking){-}[:HAS\_REVIEW]{-}\textgreater{}(rev:Review){-}[:HAS\_EMOTION]{-}\textgreater{}(em:Emotion)}
\NormalTok{    WHERE em.text IN [\textquotesingle{}joy\textquotesingle{}, \textquotesingle{}disgust\textquotesingle{}] // Filter for relevant emotions}

\NormalTok{    // Aggregate emotion counts and total bookings }
\NormalTok{    WITH a, }
\NormalTok{         em.text           AS emotion, }
\NormalTok{         count(em)         AS emotionCount, }
\NormalTok{         count(DISTINCT b) AS totalBookings}
\NormalTok{   ORDER BY a.name}

\NormalTok{   // Calculate joy{-}to{-}disgust ratio}
\NormalTok{   WITH a, }
\NormalTok{        sum(CASE WHEN emotion = \textquotesingle{}joy\textquotesingle{} THEN emotionCount ELSE 0 END) AS joyCount,}
\NormalTok{        sum(CASE WHEN emotion = \textquotesingle{}disgust\textquotesingle{} THEN emotionCount ELSE 0 END) AS disgustCount,}
\NormalTok{        totalBookings}
...
\NormalTok{   // Order by joy{-}to{-}disgust ratio to rank apartments}
\NormalTok{    ORDER BY joyDisgustRatio DESC}

\NormalTok{   // Return ranked list of apartments with total bookings}
\NormalTok{   RETURN a.name          AS apartment, }
\NormalTok{          joyDisgustRatio AS ratio, }
\NormalTok{          totalBookings}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{    // Find clusters of dissatisfaction based on negative emotions}
\NormalTok{    MATCH (a:Appartment)\textless{}{-}[:HAS\_BOOKED\_APPARTEMENT]{-}(b:Booking){-}[:HAS\_REVIEW]{-}\textgreater{}(rev:Review){-}[:HAS\_EMOTION]{-}\textgreater{}(em:Emotion)}
\NormalTok{    WHERE em.text = \textquotesingle{}disgust\textquotesingle{}}
\NormalTok{   RETURN a.name AS apartment, count(DISTINCT b) AS bookingsWithDisgust}
\NormalTok{    ORDER BY bookingsWithDisgust DESC;}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{     // Similarly for cleaning personnel}
\NormalTok{     MATCH (r:Reinigungsmitarbeiter)\textless{}{-}[:CLEANED\_BY]{-}(b:Booking){-}[:HAS\_REVIEW]{-}\textgreater{}(rev:Review){-}[:HAS\_EMOTION]{-}\textgreater{}(em:Emotion)}
\NormalTok{     WHERE em.text = \textquotesingle{}disgust\textquotesingle{}}
\NormalTok{    RETURN r.name AS cleaner, count(DISTINCT b) AS bookingsWithDisgust}
\NormalTok{     ORDER BY bookingsWithDisgust DESC;}
\end{Highlighting}
\end{Shaded}
